    ...
    public class ValidateToken
    {
            ...
            [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "validate-token")] HttpRequest req)
        {
            ...
            if (!roles.Contains("admin") && !roles.Contains("user"))
            {
                ...
                //Details of user without roles
                string _userId = principal.Claims.FirstOrDefault(c => c.Type == "oid")?.Value;
                // OR fallback directly from raw JWT:
                if (string.IsNullOrEmpty(_userId))
                {
                    var jwt = new JwtSecurityTokenHandler().ReadJwtToken(token);
                    _userId = jwt.Claims.FirstOrDefault(c => c.Type == "oid")?.Value;
                }
                var _name = principal.FindFirst("name")?.Value;
                var _email = principal.FindFirst("preferred_username")?.Value;
                _logger.LogInformation($"User: {_userId}, Name: {_name}, email:{_email}, Roles: No Roles Assigned");

                return new OkObjectResult($"User: {_userId}, Name: {_name}, email:{_email}, Roles: No Roles Assigned");
